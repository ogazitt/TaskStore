...
namespace WPKeyboardHelper
{
    ...
    public class KeyboardHelper
    {
        ...
        private Control Page
        {
            get
            {
                if (this.page == null)
                {
                    ...
                    RefreshTabbedControls(layoutRoot);
                }
                return this.page;
            }
        }
        ...
        /// </summary>
        /// <param name="layoutRoot"></param>
        public void RefreshTabbedControls(UIElement root)
        {
            if (root == null)
            {
                root = layoutRoot;
                this.page = null; // OG: hack to get the Page getter invoked by HandleReturnKey to call RefreshTabbedControls again - this time when the UIElement tree has been re-created (after control has passed back to the framework)
            }
            var l1 = GetChildsRecursive(root);
            var l2 = l1.OfType<Control>();
            var l3 = l2.Where(c => c.IsTabStop && c.TabIndex != int.MaxValue);
            var l4 = l3.OrderBy(c => c.TabIndex).ToList();
